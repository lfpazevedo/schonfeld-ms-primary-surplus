...
def load_regime_shading_data(df_pnl: pd.DataFrame) -> pd.DataFrame:
    """
    Add regime shading data to the PnL DataFrame using point-in-time regime probabilities.
    This matches the notebook's approach for visualizing high uncertainty periods.
    """
    if df_pnl.empty or "date" not in df_pnl.columns:
        return df_pnl
    
    # Ensure date column is datetime
    df_pnl["date"] = pd.to_datetime(df_pnl["date"])
    
    # Check if we already have prob_high_vol in the data (from V5/V6 CSV files)
    if "prob_high_vol" in df_pnl.columns:
        # Use the existing prob_high_vol to determine high regime periods
        df_pnl["regime_high"] = df_pnl["prob_high_vol"] > 0.5
    else:
        # Fallback: try to load from regime files
        try:
            if os.path.exists(CALENDAR_PATH) and os.path.exists(REGIME_DIR):
                cal = pd.read_csv(CALENDAR_PATH, parse_dates=["release_date"])
                cal = cal.sort_values("release_date")
                
                # For each date, find the latest regime probability file available
                regime_data = []
                for date in df_pnl["date"]:
                    available = cal[cal["release_date"] <= date]
                    if not available.empty:
                        ...
                        if os.path.exists(regime_file):
                            try:
                                ...
                                match = regime_df[regime_df["date"] == date]
                                if not match.empty and "prob_high_vol" in match.columns:
                                    prob_high = match.iloc[0]["prob_high_vol"]
                                    regime_data.append({"date": date, "prob_high_vol": prob_high})
                                    continue
                            except:
                                pass
                    regime_data.append({"date": date, "prob_high_vol": 0.5})
                
                if regime_data:
                    regime_df_merged = pd.DataFrame(regime_data)
                    df_pnl = pd.merge(df_pnl, regime_df_merged, on="date", how="left")
                    df_pnl["regime_high"] = df_pnl["prob_high_vol"] > 0.5
        except Exception as e:
            print(f"Warning: Could not load regime shading data: {e}")
            df_pnl["regime_high"] = False
...
def load_data(version="v5"):
    ...
    This function loads the daily PnL and trades data generated by the notebook's
    backtest functions (steepener_strategy_v5.py and steepener_strategy_v6.py).
    
    The CSV files contain:
    - total_pnl: Daily PnL in basis points
    - curve_pnl, gamma_pnl, carry_pnl, cost_pnl: PnL attribution components
    - prob_high_vol: Markov regime probability for high uncertainty
    - position_size: The strategy's position sizing
    - regime: Classification (high_uncertainty, medium_uncertainty, low_uncertainty)
    
    For V6, additional columns include:
    - pca1, pca_regime, prob_pca_accel: PCA-based inflation filter data
    """
    ...
    if os.path.exists(pnl_path):
        ...
        # Calculate cumulative PnL - same as notebook's .cumsum() approach
        ...
        # Add regime shading data for visualization
        df_pnl = load_regime_shading_data(df_pnl)
    else:
        df_pnl = pd.DataFrame()
...
def create_pnl_chart(df):
    """
    Create the main cumulative PnL chart with regime background shading.
    
    This matches the notebook's visualization which includes:
    - Cumulative total PnL line
    - Cumulative component attribution (curve, carry)
    - Red background shading for high uncertainty regime periods (prob_high_vol > 0.5)
    """
    if df.empty:
        return go.Figure()
    ...
    # Add regime background shading first (so it appears behind lines)
    # This matches the notebook's V5/V6 visualization style
    if "regime_high" in df.columns and df["regime_high"].any():
        # Find contiguous segments of high regime
        regime_high = df["regime_high"].values
        dates = df["date"].values
        
        # Find change points
        changes = np.where(np.diff(regime_high.astype(int)) != 0)[0] + 1
        segments = [0] + list(changes) + [len(df)]
        
        for seg_i in range(len(segments) - 1):
            s, e = segments[seg_i], min(segments[seg_i + 1], len(dates) - 1)
            if regime_high[s]:  # Only shade high regime periods
                fig.add_vrect(
                    x0=dates[s], x1=dates[e],
                    fillcolor="rgba(231,76,60,0.15)",  # Light red, matching notebook
                    line_width=0,
                    layer="below"
                )
    
    # Add PnL lines on top of shading
    ...
    fig.update_layout(
        ...
        title="Cumulative P&L Attribution (High Uncertainty Regimes Shaded)",
        title_font_size=18,
        xaxis=dict(showgrid=True, gridcolor=COLORS["gray"], zerolinecolor=COLORS["gray"]),
        yaxis=dict(showgrid=True, gridcolor=COLORS["beige"], zerolinecolor=COLORS["beige"], title="Cumulative P&L (bps)"),
        hovermode="x unified",
